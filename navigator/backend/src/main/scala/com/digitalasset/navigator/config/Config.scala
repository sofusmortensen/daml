...
object Config {
    ...
    loadSdkConfig(useDatabase).left
      .flatMap {
        case ConfigNotFound(_) =>
          logger.debug("SDK config does not exist. Falling back to Navigator config file.")
          loadNavigatorConfig(configFile, useDatabase)
        case e =>
          logger.debug(s"SDK config exists, but is not usable: ${e.reason}")
          Left(e)
      }
  }
  ...
  def loadNavigatorConfig(
    ...
    pureconfig.loadConfig[Config](config).left.map(e => ConfigParseFailed(e.toList.mkString(", ")))
  }

  def loadSdkConfig(useDatabase: Boolean): Either[ConfigReadError, Config] = {
    val partiesE = for {
      projectConfig <- ProjectConfig.loadFromEnv()
