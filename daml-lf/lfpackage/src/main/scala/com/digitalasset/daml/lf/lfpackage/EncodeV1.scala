...
import com.digitalasset.daml.lf.archive.LanguageMinorVersion
...
object EncodeV1 {
  ...
  def encodePackage(val minor: LanguageMinorVersion, pkgId: PackageId, pkg: Package) {
    PLF.Package.newBuilder().accumulateLeft(pkg.modules.sorted.values)(_ addModules _).build()
  ...
  private object KArrows
      extends RightRecMatcher[Kind, Kind]({ case KArrow(param, result) => (param, result) })
  ...
  private object TForalls
      extends RightRecMatcher[(TypeVarName, Kind), Type]({
        case TForall(binder, body) => (binder, body)
      })
  private object TApps extends LeftRecMatcher[Type, Type]({ case TApp(fun, arg) => (fun, arg) })
  ...
  private def encodeTypeBuilder(typ0: Type): PLF.Type.Builder = {
    ...
    typ match {
      case TVar(varName) =>
        builder.setVar(PLF.Type.Var.newBuilder().setVar(varName).accumulateLeft(args)(_ addArgs _))
      case TTyCon(tycon) =>
        builder.setCon(PLF.Type.Con.newBuilder().setTycon(tycon).accumulateLeft(args)(_ addArgs _))
      case TBuiltin(bType) =>
        builder.setPrim(PLF.Type.Prim.newBuilder().setPrim(bType).accumulateLeft(args)(_ addArgs _))
      case TApp(_, _) =>
  ...
  private implicit def encodeTyConApp(tyCon: TypeConApp): PLF.Type.Con =
    PLF.Type.Con.newBuilder().setTycon(tyCon.tycon).accumulateLeft(tyCon.args)(_ addArgs _).build()
  ...
  private object EApps extends LeftRecMatcher[Expr, Expr]({ case EApp(fun, arg) => fun -> arg })
  private object ETyApps extends LeftRecMatcher[Expr, Type]({ case ETyApp(exp, typ) => exp -> typ })
  private object EAbss
      extends RightRecMatcher[(ExprVarName, Type), Expr]({
        case EAbs(binder, body, _) => binder -> body
      })
  private object ETyAbss
      extends RightRecMatcher[(TypeVarName, Kind), Expr]({
  ...
  private def encodeExprBuilder(expr0: Expr): PLF.Expr.Builder = {
    ...
    expr0 match {
      ...
      case ETupleCon(fields) =>
        newBuilder.setTupleCon(PLF.Expr.TupleCon.newBuilder().accumulateLeft(fields)(_ addFields _))
      ...
      case ECase(scrut, alts) =>
        newBuilder.setCase(PLF.Case.newBuilder().setScrut(scrut).accumulateLeft(alts)(_ addAlts _))
      case ENil(typ) =>
        newBuilder.setNil(PLF.Expr.Nil.newBuilder().setType(typ))
      case ECons(typ, front, tail) =>
        newBuilder.setCons(
          PLF.Expr.Cons.newBuilder().setType(typ).accumulateLeft(front)(_ addFront _).setTail(tail))
      case ENone(typ) =>
  ...
  private implicit def encodeDataDef(nameWithDef: (DottedName, DDataType)): PLF.DefDataType = {
    ...
    dataType.cons match {
      case DataRecord(fields, _) =>
        builder.setRecord(PLF.DefDataType.Fields.newBuilder().accumulateLeft(fields)(_ addFields _))
  ...
  private implicit def encodeTemplate(nameWithTemplate: (DottedName, Template)): PLF.DefTemplate = {
    ...
    PLF.DefTemplate
      ...
      .accumulateLeft(template.choices.sorted)(_ addChoices _)
      .setObservers(template.observers)
      .accumulateLeft(template.key)(_ setKey _)
      .build()
  }

  private implicit def encodeModule(module: Module): PLF.Module = {
    ...
    def addDefinition(
      ...
      definition match {
        case x @ DDataType(_, _, cons) =>
          builder.addDataTypes(name -> x)
          cons match {
            case DataRecord(_, Some(template)) => builder.addTemplates(name -> template)
            case _ =>
          }
        case x @ DValue(_, _, _, _) =>
          builder.addValues(name -> x)
      }
      builder
    }
    ...
    PLF.Module
      ...
      .accumulateLeft(module.definitions.sorted)(addDefinition)
      .build()
  }

  def encodePackage(pkg: Package): PLF.Package =
    PLF.Package.newBuilder().accumulateLeft(pkg.modules.sorted.values)(_ addModules _).build()
...
object EncodeV1 {
  ...
  private abstract class LeftRecMatcher[Left, Right](split: PartialFunction[Left, (Left, Right)]) {
    @tailrec
    private def go(
        x: Left,
  ...
  private abstract class RightRecMatcher[Left, Right](
  ...
  private final implicit class Acc[X](val x: X) extends AnyVal {
    @inline
    def accumulateLeft[Y](iterable: Iterable[Y])(f: (X, Y) => X): X =
      iterable.foldLeft(x)(f)
    @inline
    def accumulateLeft[Y](array: ImmArray[Y])(f: (X, Y) => X): X =
      array.foldLeft(x)(f)
    @inline
    def accumulateLeft[Y](option: Option[Y])(f: (X, Y) => X): X =
      option.fold(x)(f(x, _))
  }
  ...
  private implicit class IterableOps[X, Y](iterable: Iterable[(X, Y)]) {
    def sorted: List[(X, Y)] = iterable.toList.sortBy(_._1)
